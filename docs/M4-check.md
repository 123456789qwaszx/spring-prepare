...
> **M6 이후**: 이 문서의 명령은 M4 시점 기준이라 **토큰 없이** 적혀 있다. 지금 다시 돌리려면
...
> PLAN §2.6. 결과는 §7 표에. 선행: M3 `검증됨`, 결정 **D-010** 반영됨.
...
> 그래서 한 창에서 `Body` 함수의 `deviceKey` 인자로 A·B 를 갈아 끼운다.
...
| 항목 | 기대 | 결과 | 비고 |
|---|---|---|---|
| §1 자동 테스트 | **70건** | ✅ | `SaveSlotConcurrencyTest` 가 **10라운드 전부** 통과. 구현 전에는 같은 테스트가 `expected 1 but was 2` 로 실패했다 |
| §4.1 신규 | 200, revision 1 | ✅ | |
| §4.2 재전송 | 200 **replayed:true**, revision 그대로 | ✅ | M3 에서는 여기가 409 DUPLICATE 였다 |
| §4.3 정상 | 200, revision 2 | ✅ | |
| §4.4 충돌 | **409 CONFLICT + current** | ✅ | `current.device`=device-A, `revision`=2, `playSeconds`=200 |
| §4.5 force | 200, revision 3, **acceptedChoices 1** | ✅ | 3건 보내 1건 기록. 이력 seq 1·2·3 |
| §4.6 force + 낡은 base | 409 | ✅ | `current.revision`=3 — force 는 조건을 건너뛰지 않는다 |
| §4.7 choices 없음 | 409 → (base 고치면) 200 | ✅ | D-010 이 PLAN 원문을 뒤집은 자리 |
| §4.8 base 검증 | 400 둘 | ✅ | 메시지가 각각 다른 원인을 가리킨다 |
| §5 최종 상태 | revision 4, 선택 3건, 슬롯 7 없음 | ✅ | `play_seconds` 800, `device-A` |

부수적으로 확인된 것:

- **여덟 번 요청해서 revision 은 네 번만 올랐다.**
  `신규 1 → 재전송(-) → 정상 2 → 충돌(-) → force 3 → force+낡은base(-) → choices없는충돌(-) → 정상 4`
  M3 의 F29(400 은 revision 을 안 올린다)에 이어, **409 경로도 올리지 않는다**는 것이 이 숫자로 확인됐다.
  409 는 롤백이 아니라 애초에 쓰지 않은 것이다 — 조건부 UPDATE 가 0행이면 UPDATE 자체가 아무 행도 건드리지 않았다.

- **`updated_at` 이 같은 초 안의 두 쓰기를 구별하지 못한다.** §4.1~§4.4 를 붙여넣기로 한 번에 실행했더니
  네 응답의 `updatedAt` 이 전부 `16:41:35Z` 였다. `ON UPDATE CURRENT_TIMESTAMP` 는 정상 동작하고
  (§4.5 에서 `16:42:45`, §4.7 에서 `16:42:46` 으로 갈렸다), 원인은 `DATETIME` 이 **초 단위**이기 때문이다.
  - 이것이 **"왜 시각이 아니라 revision 으로 충돌을 감지하는가"의 답**이다.
    `WHERE updated_at = :baseUpdatedAt` 로 판정했다면 같은 초의 두 쓰기가 "안 바뀐 것"으로 보여
    남의 쓰기를 조용히 덮어썼을 것이다 — M4 가 없애려던 바로 그 문제다.
  - `revision` 은 시계와 무관하게 반드시 증가한다. 서버 시계가 뒤로 가도, 같은 초에 열 번을 써도.
  - **M8 주의**: `updatedAt` 을 "언제 저장됐나"로 보여주는 것은 괜찮지만,
    두 세이브 중 어느 것이 최신인지 **판정하는 데 쓰면 안 된다.**

- **force 뒤의 이력은 한 기기의 여정이 아니다.** §4.5 에서 B 가 보낸 seq 1·2 는 걸러지고 3 만 기록됐다.
  이번에는 A 와 B 의 seq 1·2 내용이 우연히 같았지만, 실제로는 다를 수 있다 —
  `A 의 seq 2 = EP02_01/0`, `B 의 seq 2 = EP02_03/1` 이면 **B 의 것이 조용히 사라진다.**
  seq 는 슬롯 안에서 유일하되 **기기마다 독립적으로 매겨지기** 때문이다.
  PLAN 이 정의한 "A 이력 유지 + B 새 이력 추가"의 논리적 귀결이라 버그는 아니지만,
  M5 의 "선택 비율" 집계가 이 데이터를 센다는 점은 알고 있어야 한다.
  근본 해결은 seq 대신 기기별 UUID 이고, 그것은 D-010 이 미뤄 둔 방향이다 — **재검토 방아쇠가 하나 늘었다.**
