...
## D-025. "끝낸 회차" = `chapter_completed`. `ended_at` 은 통계에서 뜻이 없다 (R7, M9-1)

- 상태: **결정됨** (2026-09-02)
- 배경: 클라는 `POST /playthroughs/{id}/end` 를 **한 번도 부르지 않는다** — 갈래·이력 화면이 생긴 뒤로 회차는 "닫히는" 것이 아니라
  언제든 다시 이어 갈 수 있는 줄이다. 그래서 실사용자의 `endedPlaythroughs` 는 늘 0 이고, M5 의 "종료 회차를 분모로" 논의는 seed 에서만
  성립한다. M8 이 가져온 `save_slots.chapter_completed` 가 "끝냈다"의 실제 신호다.
- 결정:
  - 통계의 "끝낸 회차" 는 **슬롯 1 의 `chapter_completed`** 다. `GET /stats/chapters/{id}/overview` 의 `completed`·`completionRate`,
    `GET /users/{id}/summary` 의 `completedPlaythroughs` 가 그것을 센다.
  - `ended_at`·`POST …/end`·`endedPlaythroughs` 는 **남긴다**(M2 계약·테스트 8건 유지, 지우는 비용 > 두는 비용) 되 문서에 "M2 유산, 클라 미호출"로 표시.
    없애는 것은 다음 V 마이그레이션이 필요해질 때 같이.
  - 이벤트 도달률의 분모는 그대로 전체 회차(M5). "완주자 중 도달률"이 필요해지면 분모를 `chapter_completed` 로 바꾼 두 번째 지표를 더한다 — 지금 지표를 바꾸지 않는다.
- 결과: `sql/stats/chapter_overview.sql` + `ChapterOverview` + 엔드포인트 하나, `user_summary.sql` +1열, 테스트 4건(120).

---

## D-026. 챕터 안에서 에피소드 id 는 안정적이다 — 세이브 마이그레이션 API 는 만들지 않는다 (M9-2 폐기)

- 상태: **결정됨** (2026-09-02)
- 배경: M9 계획서는 "세이브를 새 콘텐츠 버전으로 옮기는 API"를 그렸다. 실물은 다르다 — PUT 이 `chapterVersion` 을 그대로 받아
  **다음 저장부터 새 버전을 가리키고**(테스트 `챕터_버전을_올려_저장하면_그_버전을_가리킨다`), 클라는 에셋 checksum 으로 버전을 찾는다(D-015).
  옮기는 API 가 할 일이 없다. 진짜 구멍은 하나: 새 버전에서 **에피소드 id 가 사라지거나 바뀌면** 큐에 그 id 의 선택이 남은 회차는
  다음 PUT 이 400(없는 에피소드)을 맞고 영영 못 올라간다.
- 결정:
  - **규칙**: 같은 `chapterId` 의 새 버전은 기존 에피소드 id 를 **지우거나 바꾸지 않는다**(추가만). 대본 쪽 규칙이지 서버가 막지 않는다 —
    막으려면 수입 시 이전 버전과 대조해야 하는데, 그건 "버전 = 바이트가 다른 파일"(D-007)과 어긋난다.
  - **클라**: PUT 이 400 `BAD_REQUEST` 로 에피소드 없음을 말하면 그 큐를 재시도하지 않고 갈라 둔다(HandleConflict 재사용) — 옛 큐 409 와 같은 처리. F5 때 같이.
  - 마이그레이션 API(M9-2)는 만들지 않는다.
- 결과: 코드 없음. Unity 답신에 규칙 한 줄.

---

## D-027. M9 를 마감 M 으로 다시 정의한다 — 관리자 화면 · `ended_at` 결정 · 회고, JPA 는 한 조각 대조만

- 상태: **결정됨** (2026-09-02)
- 배경: M9 초안(08-28)은 선택 과제 셋(JPA 재구현·세이브 마이그레이션·관리자 화면)이었다. M8 까지 오면서 둘의 전제가 바뀌었다 —
  마이그레이션은 문제가 다른 자리에 있고(D-026), JPA 통째 재구현은 같은 것을 두 번 짜는 비용 대비 남는 것이 적다.
- 결정: **M9 = 마감.** M9-1 관리자 화면(`static/admin.html`, 서버 변경은 D-025 의 개요 엔드포인트뿐) · M9-2 `ended_at` 결정(D-025) ·
  M9-3 회고(`docs/RETRO.md`) · **M9-4 JPA 한 조각 대조(선택)** — `save_slots` 만 엔티티 + `@Version`, Hibernate SQL 을 M4 의 조건부 UPDATE(F30)와
  로그로 대조, `SaveSlotConcurrencyTest` 재실행, 회차 목록의 N+1 을 M5 팬아웃과 나란히. 폐기: 세이브 마이그레이션 API, NON_NULL 전환,
  동시 POST 스레드 테스트(M4 가 같은 메커니즘을 증명). Unity 쪽 둘(옛 큐 409 갈라 두기·409 갈래 시간 재분할)은 F5 로.
- 결과: `plans/M9.md` 재작성.
