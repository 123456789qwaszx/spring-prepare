...
| R4 | `game`과 `game_test` 스키마 드리프트 | 테스트만 실패/성공 | M6 Flyway. 그 전까지 수동 동기화 | **현실화(심각)** 2026-08-29 — §4.1 참조 |
| R5 | MySQL JSON 컬럼 정규화로 "원본 그대로" 깨짐 | M1 diff ≠ 0 | PLAN 열린 결정 #1 | **해소** — D-006(JSON 유지). 실측 5,686→3,581(37%), 의미 비교로 검증 |
| R6 | 컴파일 없이 작성하는 방식이 M이 커질수록 비용이 커짐 | 코드량 증가 시 첫 빌드 실패율 상승 | 스텁 컴파일로 사전 검증 | **낮음으로 하향** — M1(17파일, 트랜잭션·배치·Jackson3)도 컴파일 오류 0. 다만 이는 구현 전 프레임워크 소스를 대조했기 때문이며(§4.2), 그 단계를 건너뛰면 위험은 그대로다 |
