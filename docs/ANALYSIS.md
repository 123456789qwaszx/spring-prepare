...
| # | 리스크 | 징후 | 대응 |
|---|---|---|---|
| R1 | 컴파일 없이 쓴 코드의 오타·시그니처 오류 | 첫 `bootRun`에서 컴파일 에러 | 에러 메시지 전체를 붙여 주면 즉시 수정. 계획서에 "첫 빌드는 실패할 수 있다"를 전제로 둠 |
| R2 | Boot 4.1.1 복귀 시 Gradle 의존성 해석 실패 | `Could not resolve …webmvc` | PLAN §2.2 지시대로 추측 변경 금지, 릴리스 노트 확인. 스타터 이름은 소스로 확인했으므로 낮은 확률 |
| R3 | 기존 실습1·2 코드가 Boot 4에서 깨짐 | Thymeleaf·Lombok 관련 컴파일 에러 | Thymeleaf 스타터는 4.x에 존재, Lombok은 BOM 관리. 깨지면 실습 코드는 최소 수정 |
| R4 | `game`과 `game_test` 스키마 드리프트 | 테스트만 실패/성공 | M6 Flyway 결정 시 두 DB 자동 적용. 그 전까지는 M0-check.md 절차로 수동 동기화 |
| R5 | MySQL JSON 컬럼 정규화로 "원본 그대로" 깨짐 | M1 diff ≠ 0 | PLAN 열린 결정 #1. M1 착수 시 결정 |
