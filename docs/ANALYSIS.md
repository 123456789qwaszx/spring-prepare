...
> **2026-08-29 M3 착수 시 해소 — D-009.** 다만 이 항목이 처음에 적은 그림은 **절반만 맞았다.**
> Connector/J 9.7.0 문서를 실제로 읽고 나서 밝혀진 것 둘:
>
> 1. **`serverTimezone=Asia/Seoul`은 세션 `time_zone`을 설정한 적이 없다.** `forceConnectionTimeZoneToSession`의 기본값이 `false`이기 때문이다(F24). M0에서 `created_at`이 KST로 맞았던 것은 **URL이 그렇게 시켰기 때문이 아니라 MySQL 서버의 기본 시간대가 KST였기 때문**이다. 맞는 결과를 보고 틀린 원인을 믿고 있었던 셈이다.
> 2. **변환은 "조용히 일어나는" 것이 아니라 한쪽에서만 일어난다**(F23). `DATETIME`에 **쓸 때는 변환이 없고**, 읽을 때만 있다. 그래서 `OffsetDateTime`을 그대로 넘기면 오프셋이 버려진 벽시계가 저장된다 — 변환이 일어나서 틀리는 게 아니라 **일어나지 않아서** 틀린다.
>
> 결론: 앱이 쓰기 직전에 직접 UTC로 정규화하고(`common/UtcTime`), URL에 `connectionTimeZone=UTC&forceConnectionTimeZoneToSession=true`를 건다.
> 교훈은 §4.2와 같은 방향이다 — **동작하는 결과가 이해가 맞다는 증거는 아니다.**
...
| R4 | `game`과 `game_test` 스키마 드리프트 | 테스트만 실패/성공 | M6 Flyway. 그 전까지 수동 동기화 | **해소** 2026-08-31 — D-012 재생성 + Flyway. `flyway_schema_history` V1~V5 양쪽 확인, 테스트 기동이 game_test 를 자동 마이그레이션(M6 F42). §4.1 은 역사 기록으로 남는다 |
| R5 | MySQL JSON 컬럼 정규화로 "원본 그대로" 깨짐 | M1 diff ≠ 0 | PLAN 열린 결정 #1 | **해소** — D-006(JSON 유지). 실측 5,686→3,581(37%), 의미 비교로 검증 |
| R6 | 컴파일 없이 작성하는 방식이 M이 커질수록 비용이 커짐 | 코드량 증가 시 첫 빌드 실패율 상승 | 스텁 컴파일로 사전 검증 | **낮음 유지** — M6(신규·수정 45파일, 인터셉터·보안·마이그레이션)도 첫 실행 96/96. 이번엔 소스 대조에 더해 **전 소스(main+test)를 스텁 javac 로 타입 검사**했다. 두 단계 중 하나라도 건너뛰면 위험은 그대로다(§4.2) |

### 4.1 R4가 실제로 터진 방식 (2026-08-29)

M1의 마이그레이션 단계에서 `ALTER TABLE game.game_definitions` 가 `Error 1146: Table doesn't exist` 로 실패했다. 확인해 보니:

- `game`: **`users` 하나뿐** (8개 누락)
- `game_test`: 아홉 개 정상

즉 드리프트가 "컬럼 하나 차이" 수준이 아니라 **테이블 여덟 개 누락**이었고, M0는 `users`만 썼기 때문에 완료 기준 7개를 전부 통과하고도 드러나지 않았다. M1에 와서야 터졌다.

여기서 두 가지가 확인됐다.

1. **부트스트랩 스크립트가 멱등하지 않다.** `schema.sql` 은 `CREATE TABLE`(`IF NOT EXISTS` 없음)이라 통째로 재실행하면 기존 테이블에서 멈춘다. "지금 상태에 맞춰 보정"이 불가능해서, 복구하려면 그때그때 손으로 SQL을 짜야 했다 (M1 §7-1 F14).
2. **선행 조건 확인은 자동으로 드러나지 않는다.** M0-check §0.1에 "아홉 개가 보이는지 확인"이라고 적었지만, 확인하지 않아도 M0는 통과한다. 완료 기준과 달리 선행 조건은 실패가 조용하다.

대응: 이후 M의 선행 조건에는 확인 쿼리와 기대 출력을 함께 적는다. 근본 해결은 M6 Flyway — `game`·`game_test` 양쪽에 같은 마이그레이션이 자동 적용되고, 적용 이력이 `flyway_schema_history` 에 남아 드리프트가 조용히 생기지 못한다.

### 4.2 무컴파일 작성이 M1에서도 성립한 이유

M1은 코드 17파일에 트랜잭션·배치 INSERT·Jackson 3가 얽혔는데도 첫 `compileTestJava`가 통과했다. 운이 아니라 구현 **전에** 프레임워크 소스를 직접 대조했기 때문이다. 그 과정에서 나온 것:

- Jackson 3에서 `asText()`·`textValue()` 는 `@Deprecated` → `asString()`·`stringValue()`. 인자 없는 `asString()` 은 값이 없으면 예외를 던지므로 `asString("")` 이 안전한 형태다.
- `JacksonException` 이 `RuntimeException`(2.x는 checked) → try/catch 불필요, 그리고 파싱 실패가 `@Transactional` 롤백에 걸린다.
- `StringHttpMessageConverter` 의 기본 charset 은 ISO-8859-1 이지만 `application/json` 에는 UTF-8 예외 규칙이 있다 → 원본 JSON 을 그대로 돌려주는 GET 이 이 규칙에 의존한다.

셋 다 **컴파일은 통과하되 런타임에 조용히 틀렸을** 것들이다. 이 대조 단계를 건너뛰면 R6는 다시 올라간다.

### 4.3 M3에서 이 습관이 잡아낸 것 — "맞는 결과, 틀린 이해"

M3는 앞선 사례와 종류가 다르다. M1의 셋은 **아직 안 쓴 API**를 확인한 것이지만, M3의 것은 **이미 통과한 M0의 결과를 다시 의심한** 경우다.

M0에서 `created_at`이 KST로 정확히 기록됐고(F8), 우리는 그것을 "URL의 `serverTimezone=Asia/Seoul`이 일하고 있다"로 읽었다. 코드를 쓰기 전에 Connector/J 문서를 대조하고 나서야, `forceConnectionTimeZoneToSession`의 기본값이 `false`라 **그 URL은 세션 시간대를 설정한 적이 없다**는 것이 드러났다(F24). 맞는 결과가 나온 이유는 MySQL 서버 기본값이 우연히 KST였기 때문이다.

여기서 멈추지 않았다면 M3는 이렇게 실패했을 것이다: 앱이 넣는 `chosen_at`은 UTC로, DB DEFAULT가 채우는 `received_at`·`created_at`은 KST로 들어가 **한 테이블 안에서 시간대가 갈린다.** 두 값 다 "그럴듯한 시각"이라 눈으로는 잡히지 않고, 왕복 테스트도 통과한다.

- **초록색은 이해의 증거가 아니다.** F21(가짜 초록)이 절차 층위의 같은 이야기였다면, 이것은 이해 층위의 같은 이야기다.
- 그래서 M3 테스트는 왕복이 아니라 **`DATE_FORMAT`으로 DB의 벽시계 문자열을 직접** 본다. 쓰기와 읽기가 같은 방향으로 틀리면 왕복만으로는 영원히 잡히지 않기 때문이다(M3.md C8).
...
- 2026-08-29 M1 검증 후: R5 해소(D-006), R6 하향, **R4 현실화 상세를 §4.1에 기록**(테이블 8개 누락), 무컴파일 작성의 성립 조건을 §4.2에 기록. §3.9의 "schema.sql은 DDL 정본"에 한 가지를 덧붙인다 — 정본이되 **재실행 가능한 스크립트는 아니다**.
- 2026-08-31 M6 검증 후: **R4 해소** — Flyway 실동작으로. §4.2 의 목록에 M6 의 셋이 추가된 셈이다(starter-flyway 존재, encoding 기본값, MVC 예외의 ErrorResponse 인터페이스 — D-012 갱신 3·4, F43). 소스 대조가 이번엔 "초안의 자기 교정"까지 뒤집었다: 없다고 배운 이름(starter-flyway)이 버전이 바뀌며 생겼다.
- 2026-08-29 M3 착수 시: §3.8을 D-009로 해소하면서 **이 항목이 절반만 맞았음을 기록**(쓰기에는 변환이 없다, `serverTimezone`은 세션을 설정한 적이 없다). §4.3 신설 — 소스 대조가 "아직 안 쓴 API"뿐 아니라 "이미 통과한 결과"도 의심하게 해 준 사례.
