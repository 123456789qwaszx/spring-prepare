...
```powershell
Call-Api POST "http://localhost:8080/users" '{"username":"amiya","password":"pw"}'
```
기대: `201` / `{"id":<번호>,"username":"amiya"}` — **id를 적어 둔다.**

> **번호가 1이 아닐 수 있다** (§1.4). `DELETE` 는 AUTO_INCREMENT 를 되돌리지 않는다.
> 아래 `$USER_ID` · `$PT` 에는 **문서의 숫자가 아니라 응답이 준 값**을 넣는다.

```powershell
$USER_ID = 1                      # ← 위 응답의 id 로 바꿀 것
Call-Api POST "http://localhost:8080/users/$USER_ID/playthroughs"
```
기대: `201` / `{"playthroughId":<번호>}`

```powershell
$PT = 1                                          # ← 위 응답의 playthroughId 로 바꿀 것
"확인: http://localhost:8080/playthroughs/$PT/saves/1"
# playthroughs/1/saves/1  → 정상
# playthroughs//saves/1   → $PT 가 비어 있다. 이 줄을 건너뛰면 이후가 전부 404다
```
...
{"chapterId":"qwer-events","chapterVersion":1,"currentEpisodeId":"EP04_01",
 ...
 "choices":[
   ...
   {"seq":2,"episodeId":"EP02_01","optionIndex":0,"chosenAt":"2026-08-29T20:41:23Z"},
   {"seq":3,"episodeId":"EP03_01","optionIndex":1,"chosenAt":"2026-08-29T20:42:31Z"}
 ],
 "events":[{"episodeId":"EP04_01","occurredAt":"2026-08-29T20:43:47Z"}]}
"@
...
| seq | chosen_at_raw | 왜 |
|---|---|---|
| 1 | `2026-08-29 11:40:19` | 보낸 값은 `20:40:19+09:00`. **9시간 빠진 UTC** |
| 2 | `2026-08-29 11:41:23` | 보낸 값은 `20:41:23Z` — 그대로 |
| 3 | `2026-08-29 11:42:31` | 그대로 |
...
| §1 데이터 폐기 | 9개 테이블 전부 0 | | |
| §2 자동 테스트 | **61건** | | `cleanTest` 필수 |
| §3 bootRun | 기동 성공 | | |
| §4.2 fixture 수입 | 201, EventKey 3개 | | |
| §4.3 선택 3 + 이벤트 1 | 200, accepted 3/1, `updatedAt` 에 `Z` | | |
| §4.4 없는 에피소드 | **400 BAD_REQUEST**, revision 불변 | | `CONSTRAINT_VIOLATION` 이면 사전 검증 실패 |
| §4.5 EventKey 없음 | 400 | | |
| §4.6 seq 재전송 | **409 DUPLICATE**, revision 불변 | | M4에서 200 replayed로 바뀔 자리 |
| §4.7 조회 | events 1건 + 표시명, choices 3 → 1 | | |
| §4.8 이력 없는 PUT | 200, accepted 0/0 | | |
| §5.1 시각 | seq 1이 `11:40:19` | | **9시간 어긋나면 D-009 실패** |
| §5.2 시간대 일치 | DEFAULT와 앱 값이 같은 시간대 | | |
| §5.3 원자성 | 선택 3, 이벤트 1, play_seconds 600 | | |
| §5.4 복합 FK JOIN | 1행, 표시명·제목 나옴 | | |
